  ...
  return {
    addClass: function addClass(className) {
      for (var _i = 0; _i < elements.length; _i++) {
        elements[_i].classList.add(className);
      }

      return this; //链式操作，这样可继续在此基础上操作
    ...
    find: function find(selector) {
      ...
      for (i = 0; i < elements.length; i++) {
        var x = Array.from(elements[i].querySelectorAll(selector)); //注意要用Array.from把后面的伪数组变成数组，
        //否则会有bug
    ...
    children: function children() {
      ...
      this.each(function (node) {
        if (array.indexOf(node.childrenNode) === -1) {
          array.push.apply(array, _toConsumableArray(node.childrenNode));
